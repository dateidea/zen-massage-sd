  ...
  return (
    <li className="border-t border-hairline">
      <button aria-expanded={open} aria-controls={id} onClick={() => setOpen((s) => !s)} className="group flex w-full items-start justify-between gap-6 py-7 text-left">
        <span className={`display text-[22px] leading-[1.25] md:text-[26px] ${open ? "text-clay" : "text-ink"}`}>{qa.q}</span>
        <span aria-hidden className={`mt-2 inline-block h-3 w-3 shrink-0 transition-transform duration-500 ${open ? "rotate-45" : ""}`} style={{ background: "linear-gradient(currentColor, currentColor) center/100% 1px no-repeat, linear-gradient(currentColor, currentColor) center/1px 100% no-repeat", color: "var(--color-clay)" }} />
      </button>
      <div id={id} role="region" className="grid transition-[grid-template-rows] duration-700 ease-out" style={{ gridTemplateRows: open ? "1fr" : "0fr" }}>
        ...
        <Reveal>
          <div className="grid grid-cols-12 gap-6">
            ...
            <div className="col-span-12 md:col-span-8">
              <h2 id="faq-heading" className="display text-[40px] leading-[1.04] text-ink md:text-[56px]">The honest answers,<br /><span className="italic font-light text-clay">before you have to ask.</span></h2>
            </div>
          </div>
        </Reveal>
        <Reveal>
