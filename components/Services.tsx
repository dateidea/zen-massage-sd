...
export default function Services() {
  return (
    <section id="services" className="bg-cream-deep py-24 md:py-32" aria-labelledby="services-heading">
      <div className="mx-auto max-w-[1320px] px-6 md:px-10">
        <Reveal>
          <div className="grid grid-cols-12 gap-6">
            <div className="col-span-12 md:col-span-4"><p className="eyebrow">The menu</p></div>
            <div className="col-span-12 md:col-span-8">
              <h2 id="services-heading" className="display text-[40px] leading-[1.04] text-ink font-medium md:text-[60px]">
                Four services.<br />
                <span className="text-clay-deep font-normal">No memberships, no add-on traps,</span><br />
        ...
        <div className="mt-16 grid grid-cols-1 gap-6 md:mt-20 md:grid-cols-2 md:gap-8">
          {services.map((s, i) => (
            <Reveal key={s.num} delay={i * 80}>
              <article className="card flex h-full flex-col overflow-hidden p-6 md:p-8">
                <div className="card-img relative aspect-[5/4] w-full overflow-hidden">
                  <div role="img" aria-label={s.alt} className="img-placeholder image-soft absolute inset-0" style={{ backgroundImage: `url(${asset(s.image)})`, backgroundSize: "cover", backgroundPosition: "center" }} />
                  <span className="absolute left-4 top-4 inline-flex items-center rounded-full bg-cream/90 backdrop-blur px-3 py-1 text-[11px] tracking-[0.16em] uppercase text-ink-soft">{s.num}</span>
                </div>
                <div className="mt-6 flex items-baseline gap-4">
                  <h3 className="display text-[26px] leading-[1.05] text-ink font-medium md:text-[32px]">{s.name}</h3>
                  <span className="ml-auto display text-[18px] text-clay-deep font-medium md:text-[20px]">{s.price}</span>
                </div>
                <p className="mt-2 text-[12px] uppercase tracking-[0.16em] text-mid">{s.duration}</p>
                <p className="mt-5 text-[15px] leading-[1.65] text-ink-soft">{s.description}</p>
                <p className="mt-3 text-[13px] italic text-mid">{s.outcome}</p>
                <a href="#booking" className="mt-6 link-underline self-start text-[13px] tracking-[-0.005em] text-clay-deep font-medium">Book this →</a>
              </article>
            </Reveal>
          ))}
        </div>
