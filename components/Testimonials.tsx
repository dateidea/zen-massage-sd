...
export default function Testimonials() {
  return (
    <section aria-labelledby="testimonials-heading" className="bg-shadow py-24 text-ink md:py-32">
      <div className="mx-auto max-w-[1320px] px-6 md:px-10">
        <Reveal>
          <div className="grid grid-cols-12 gap-6">
            <div className="col-span-12 md:col-span-4"><p className="eyebrow">In their words</p></div>
            <div className="col-span-12 md:col-span-8">
              <h2 id="testimonials-heading" className="display text-[40px] leading-[1.04] md:text-[64px]">People who walked in<br /><span className="italic font-light text-clay">on a hard day,</span><br />and came back the next.</h2>
        ...
        <div className="mt-20 grid grid-cols-12 gap-x-6 gap-y-16">
          {quotes.map((q, i) => (
            <Reveal key={q.name} delay={i * 100} className={i === 0 ? "col-span-12 md:col-span-8" : i === 1 ? "col-span-12 md:col-span-5 md:col-start-1" : "col-span-12 md:col-span-6 md:col-start-7"}>
              <figure className="flex flex-col">
                <blockquote className={`display leading-[1.18] text-ink ${i === 0 ? "text-[28px] md:text-[40px]" : "text-[22px] md:text-[28px]"}`}>
                  <span className="text-clay">&ldquo;</span>{q.body}<span className="text-clay">&rdquo;</span>
                </blockquote>
                <figcaption className="mt-8 flex items-center gap-4">
                  <span aria-hidden className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-clay/40 text-[12px] tracking-[0.18em] text-clay">{initials(q.name)}</span>
        ...
        <Reveal>
          <div className="mt-20 flex flex-wrap items-center gap-x-8 gap-y-3 border-t border-hairline pt-10 text-[13px] text-ink-soft">
            <span className="display text-[28px] text-clay">★★★★★</span>
