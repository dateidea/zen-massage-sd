...
export default function Process() {
  return (
    <section id="process" className="bg-cream-deep py-24 md:py-32" aria-labelledby="process-heading">
      <div className="mx-auto max-w-[1320px] px-6 md:px-10">
        <Reveal>
          <div className="grid grid-cols-12 gap-6">
            <div className="col-span-12 md:col-span-4"><p className="eyebrow">How it works</p></div>
            <div className="col-span-12 md:col-span-8">
              <h2 id="process-heading" className="display text-[40px] leading-[1.04] md:text-[64px]">Four steps.<br /><span className="italic font-light text-clay">Mostly the same as</span><br />your last good massage.</h2>
            </div>
          </div>
        </Reveal>
        <ol className="mt-20 grid grid-cols-12 gap-x-6 gap-y-12 md:gap-y-16">
          {steps.map((s, i) => (
            <Reveal key={s.n} as="li" delay={i * 80} className="col-span-12 md:col-span-6">
              <div className="flex flex-col">
                <div className="flex items-baseline gap-6">
                  <span className="display text-[64px] leading-none text-clay md:text-[88px]">{s.n}</span>
                  <span className="h-px flex-1 bg-hairline" />
                </div>
                <h3 className="display mt-6 text-[26px] leading-[1.15] text-ink md:text-[32px]">{s.title}</h3>
